...
class EvictionPolicyModel(nn.Module):
  ...
  @classmethod
  def set_exp_dir(cls, exp_dir=None):
      """Sets the value of exp_dir."""
      cls.exp_dir = exp_dir

  def save_metadata(self, cache_accesses, attention_weights, scores, batch_size):
    metadata = []
    filename = os.path.join(EvictionPolicyModel.exp_dir, f"attention.pkl")
    ...
    for i in range(batch_size):
        metadata_entry = {
            "pc": cache_accesses[i].pc,
            "address": cache_accesses[i].address,
            "set_id": cache_accesses[i].set_id,
            "cache_lines": cache_accesses[i].cache_lines,
            "access_history": cache_accesses[i].access_history,
            "attention_weights": attention_weights[i].detach().numpy(), 
            "scores": scores[i].detach().numpy()
        }
  ...
  def forward(self, cache_accesses, prev_hidden_state=None, inference=False, batch_id=None):
    ...
    self.save_metadata(cache_accesses, attention_weights, scores, batch_size)
