...
def evaluate(policy_model, data, step, descriptor, tb_writer, log_dir, k=5):
  ...
  for batch_id, batch in tqdm.tqdm(enumerate(zip(*subsequences)), desc=desc, total=subseq_length):
    probs, pred_reuse_distances, hidden_state, attention = policy_model(
        [entry.cache_access for entry in batch], hidden_state, inference=True, batch_id=batch_id)
...
def main(_):
  ...
  if EvictionPolicyModel.exp_dir is None:
    EvictionPolicyModel.set_exp_dir(exp_dir=exp_dir)
